%% Incremental Site Assessment Procedure (ISAP) Costings
...
% Per Site
DESKTOP.PerSite.Cost_Charts             = 200;                              % ADMIRALTY / Navionics high resolution charts.
...
INITIAL.Baseline.Cost_SonarBuoy          = 100;                             % Deeper/Fishhunter
INITIAL.Baseline.Cost_CTDSonde           = 500;
...
disp('Calculating total costs at each stage of the ISAP (variable success rates)...');
% Desktop
...
% ISAP
COSTS1.ISAP.NumberSitesRemaining...
    = ceil(COSTS1.DETAILED.NumberSites2Assess...
    .* (1 - IN.DETAILED.SuccessRate));


COSTS1.ISAP.Total = COSTS1.DESKTOP.Total...
...
disp('Calculating total costs at each stage of the ISAP (constant success rate)...');
% Desktop
...
% ISAP
COSTS2.ISAP.NumberSitesRemaining...
    = ceil(COSTS2.DETAILED.NumberSites2Assess...
    .* (1 - IN.DETAILED.SuccessRate'));


COSTS2.ISAP.Total = COSTS2.DESKTOP.Total...
...
% Ticks
c1.Limits = [round2(min(COSTS2.ISAP.Total,[],'all'),max(diff(c1.Ticks)/2)) ...
            round2(max(COSTS2.ISAP.Total,[],'all'),max(diff(c1.Ticks)/2))];
...
% Ticks
c2.Limits = [round2(min(COSTS2.ISAP.Total,[],'all'),max(diff(c2.Ticks)/2)) ...
            round2(max(COSTS2.ISAP.Total,[],'all'),max(diff(c2.Ticks)/2))];
...
%% ISAP vs Standard Surface
figure;
S1 = surf(IN.NumberPotentialSites2Assess,...
       IN.CONSTANT.SuccessRate'.*100,...
       COSTS2.ISAP.Total); hold on;
...
ZTicks = 0:...
    (max(diff(zticks))/2):...
    round2(max(COSTS2.ISAP.Total,[],'all'),max(diff(zticks))/2);
...
%% ISAP vs Standard Plot (50% Success Rate)
figure;
plot(IN.NumberPotentialSites2Assess,...
     COSTS2.ISAP.Total((IN.CONSTANT.SuccessRate == 0.5),:),'-o'); hold on;
plot(IN.NumberPotentialSites2Assess,...
...
title('Incremental vs Standard Site Assessment Procedure - Total Costs','FontSize',FontSize_Title); 
legend('ISAP (50% Success Rate)','Standard Procedure','Location','northwest');
